...
int main(int argc, char* argv[]) {
  ...
  if (argc != 2)                                                                                                                                                                                                       pythia.readFile("main04.cmnd");                                                                                                                                                                                  else {                                                                                                                                                                                                               // Check that the provided input name corresponds to an existing file.
    ifstream is(argv[1]);                                                                                                                                                                                              if (!is) {                                                                                                                                                                                                           cerr << " Command-line file " << argv[1] << " was not found. \n"                                                                                                                                                        << " Program stopped! " << endl;                                                                                                                                                                              return 1;                                                                                                                                                                                                        }
    pythia.readFile(argv[1]);                                                                                                                                                                                        }
  // Extract settings to be used in the main program.
  int    nEvent    = pythia.mode("Main:numberOfEvents");                                                                                                                                                             int    nAbort    = pythia.mode("Main:timesAllowErrors");
  // Initialize.
  pythia.init();
  ...
  T->Branch("Q2", &Q2, "Q2/F");
  T->Branch("W2", &W2, "W2/F");
  T->Branch("xpom", &xpom, "xpom/F");
  T->Branch("tpom", &tpom, "tpom/F");
  ...
  for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
    ...
    //xpom =-999;
    // tpom = -999;
    //if(pythia.info.isHardDiffractiveA() == 1 || pythia.info.isHardDiffractiveB() == 1){
      // std::cout << "hard diffractive" << std::endl;
      //std::cout << pythia.info.xPomeronA() << " " << pythia.info.tPomeronA() << std::endl;
    // xpom = pythia.info.xPomeronA();
      // tpom = pythia.info.tPomeronA(); 
      // }
      //else {continue;}
    //else{ std::cout << " not diffractive " << std::endl;}
	...
	( fastjet::SelectorIsHadron() && fastjet::SelectorIsNeutral())(constituents);
      ...
      dphi_e_jet.push_back(0);//TMath::Abs(TVector2::Phi_mpi_pi(jet.phi_std()-e_phi)));


      Float_t deta = jet.eta()-quark_eta;
      Float_t dphi = 0;//TMath::Abs(TVector2::Phi_mpi_pi(jet.phi_std()-quark_phi));
      Float_t dR = TMath::Sqrt(dphi*dphi + deta*deta);

      dR_q_jet.push_back(dR);
      dphi_q_jet.push_back(0);// TMath::Abs(TVector2::Phi_mpi_pi(jet.phi_std()-quark_phi)));
      ...
      jet_p.push_back(sqrt(jet.modp2()));
